...
const ActiveLink: React.FC<PropsWithChildren<ActiveLinkProps>> = ({
  ...
  className = "",     
  href,               
  ...props            // Collect remaining LinkProps (like target, rel, etc.)
}) => {
  ...
  const isActive = hrefString === '/' ? asPath === hrefString : asPath.startsWith(hrefString);
  ...
  return (
    <Link 
      ...
      {...props} // Spread the rest of the LinkProps
    >
      {children}
    </Link>
  );
